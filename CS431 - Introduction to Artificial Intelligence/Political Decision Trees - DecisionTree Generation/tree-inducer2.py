...
NOTE: This decision tree does not appear to function correctly. It has generally low accuracy, which ends up
blatantly wrong in small scale. However, the resulting tree structure it generates looks more accurate than
the other version, which is very short.
...
def classify(data_point, tree):
    ...
    return current.get_classification("")
...
def induce_node_tree(data_set, issues, prev_majority, level):
    ...
    if democrats == 0: #if there are republicans but no democrats
        return Node(is_leaf = True, classification = "D", depth = level)
    if republicans == 0: #if there are democrats but no republicans
        return Node(is_leaf = True, issue = None, classification = "R", depth = level)
    ...
    branch = Node(is_leaf=False, issue=question, classification = maj, yea=yea_path, nay=nay_path, abstain=abstain_path, depth=level)
...
class Node:
    ...
    def __init__(self, is_leaf=True, issue=None, classification=None, yea=None, nay=None, abstain=None, depth=0):
    ...
    def get_classification(self, vote):
        if self.is_leaf: return self.classification
